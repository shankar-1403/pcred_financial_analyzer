...
def extract_account_info(lines):
    ...
    for line in lines:
        ...
        if info["account_holder"] is None:
            ...
            # 2️⃣ fallback for Axis-style statements
            if info["account_holder"] is None:
                clean = (line or "").strip()
                ...
                if (
                    ...
                    and clean.isupper()
                    and len(clean) > 5
                    and "bank" not in clean.lower()
                    and "statement" not in clean.lower()
                    and "account" not in clean.lower()
                ):
...
def _row_to_txn(row, column_mapping, last_txn):
    ...
    if not date_cell or not re.search(r"\d{2}[-/]\d{2}[-/]\d{4}", str(date_cell)):
        return None
        ...
        for page in pdf.pages:
            ...
            for table in tables:
                ...
                for row in table:
                    ...
                    # first time header detection
                    if detected and column_mapping is None:
                        column_mapping = detected
                        continue

                    # skip only if header repeats with same structure
                    if detected and detected == column_mapping:
                        continue
