"""
ICICI Bank statement parsing.
Edit extract_account_info and extract_transactions below for ICICI-specific logic.
Uses separate withdrawal/debit and deposit/credit columns.
"""
import re
import pdfplumber
from .base import (
...
def _row_looks_like_header(row, column_mapping):
    """Skip row if date/description cells contain header keywords (don't add header as first entry)."""
    try:
        if "date" in column_mapping and column_mapping["date"] < len(row):
            cell = (row[column_mapping["date"]] or "").strip().lower()
            if cell in ("date", "txn date", "value date", "transaction date", "transaction date & time"):
                return True
        if "description" in column_mapping and column_mapping["description"] < len(row):
            ...
            if any(h in cell for h in ("description", "narration", "particulars", "details", "transaction remarks")):
                return True
...
def _row_to_txn(row, column_mapping, last_txn):
    """ICICI: separate debit and credit columns."""
...
def extract_transactions(pdf_path: str):
    """Extract transactions from ICICI PDF (separate debit/credit columns). Edit table logic above if needed."""
    transactions = []
    column_mapping = None
    last_txn = None
