"""
Generic/fallback bank statement parsing when bank is not identified.
Edit extract_account_info and extract_transactions below; bank name is detected from content.
"""
import re
import pdfplumber
from .base import (
...
# --- Bank detection from content (edit to add more banks) ---
BANK_PATTERNS = {
...
# --- Account info patterns (edit as needed) ---
ACCOUNT_PATTERNS = [
...
def extract_account_info(lines):
    """Extract account metadata from text lines. Bank name from content. Edit patterns above if needed."""
...
def _row_to_txn(row, column_mapping, last_txn):
    """Generic: separate debit and credit columns."""
...
def extract_transactions(pdf_path: str):
    """Extract transactions from PDF (separate debit/credit columns). Edit table logic above if needed."""
    transactions = []
    column_mapping = None
    last_txn = None
